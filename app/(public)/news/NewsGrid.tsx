...
import type { NewsItem, Category } from "@/lib/supabase-data";
...
export default function NewsGrid({ news, categories }: NewsGridProps) {
  ...
  const newsItems = filteredNews.slice(startIndex, startIndex + ITEMS_PER_PAGE);
  ...
  const updateUrl = useCallback((category: string | null, page: number) => {
    ...
    router.push(queryString ? `/news?${queryString}` : "/news", { scroll: false });
  }, [router]);
